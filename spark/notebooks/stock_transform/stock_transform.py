...
if __name__ == '__main__':
    ...
    def app():
        ...
        spark = SparkSession.builder.appName("FormatStock") \
            ...
            .config("fs.s3a.endpoint", os.getenv("ENDPOINT", "http://172.23.0.4:9000")) \
...
#         path = "stock-market/NVDA"  # <-- Ganti ini sesuai kebutuhan

#         df = spark.read.option("header", "false") \
#             .json(f"s3a://{path}/prices.json")
...
#             .option("header", "true") \
#             .option("delimiter", ",") \
#             .csv(f"s3a://{path}/formatted_prices")
